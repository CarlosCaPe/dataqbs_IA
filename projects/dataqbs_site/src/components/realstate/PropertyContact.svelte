    ...
    try {
      const res = await fetch('/api/contact', {
        ...
        body: JSON.stringify({
          name: name.trim(),
          email: email.trim(),
          message: `[realstate — ${propertyId}] ${propertyTitle}\n\n${message.trim()}`,
          locale: $locale,
  ...
  <div class="grid gap-5 lg:grid-cols-2">
      ...
      {#if sent}
        <div class="text-center py-8 animate-fade-in">
          ...
          <button on:click={() => (sent = false)}
            class="mt-4 px-4 py-2 rounded-lg text-sm font-medium text-rs-600 dark:text-rs-400 border border-rs-300 dark:border-rs-700 hover:bg-rs-50 dark:hover:bg-rs-900/20 transition-colors">
            ← {$t.rs.requestInfo}
          </button>
        </div>
      {:else}
        <form on:submit|preventDefault={handleSubmit} class="space-y-4">
          ...
          <div class="flex items-center gap-2 px-3 py-2 rounded-lg bg-rs-50 dark:bg-rs-900/20 border border-rs-200 dark:border-rs-800/40 text-sm">
            <span class="text-rs-600 dark:text-rs-400">🏠</span>
            <span class="text-rs-700 dark:text-rs-300 font-medium truncate">{propertyTitle}</span>
          </div>

          <div>
            <label for="rs-name" class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">
              {$t.contact.nameLabel}
            </label>
            <input
              id="rs-name"
              ...
              class="w-full px-3 py-2 rounded-lg text-sm bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 text-slate-900 dark:text-white placeholder-slate-400 focus:ring-2 focus:ring-rs-500 focus:border-rs-500 outline-none transition-colors"
              required
            />
          </div>
          <div>
            <label for="rs-email" class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">
          ...
          <div>
            <label for="rs-message" class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">
            ...
            <textarea
              ...
              rows="4"
              class="w-full px-3 py-2 rounded-lg text-sm bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 text-slate-900 dark:text-white placeholder-slate-400 focus:ring-2 focus:ring-rs-500 focus:border-rs-500 outline-none transition-colors resize-none"
              required
          ...
          <button type="submit"
            class="w-full inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg font-semibold text-sm bg-rs-600 text-white hover:bg-rs-700 active:bg-rs-800 disabled:opacity-50 transition-colors"
            disabled={sending}>
            {#if sending}
    ...
    <div class="p-5 rounded-xl bg-slate-50 dark:bg-slate-700/40 border border-slate-200 dark:border-slate-700 space-y-3">
      ...
      <a
        ...
        class="flex items-center gap-3 p-3 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-700/50 transition-colors group"
      >
        <div class="w-10 h-10 rounded-full bg-rs-100 dark:bg-rs-900/30 flex items-center justify-center text-rs-600 dark:text-rs-400 group-hover:scale-110 transition-transform">
