<script lang="ts">
  ...
  async function handleSubmit() {
    ...
    if (message.length > maxMessageLength) {
      errorMsg = $t.contact.messageTooLong?.replace('{max}', String(maxMessageLength)) || `Message too long (max ${maxMessageLength} characters)`;
      return;
    }
  ...
  const fillIcons = new Set(['GitHub', 'LinkedIn', 'WhatsApp']);
</script>
        ...
        <form on:submit|preventDefault={handleSubmit} class="space-y-4">
          ...
          {#if hasTranscript}
            <div class="flex items-center gap-2 text-xs text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-900/20 px-3 py-2 rounded-lg border border-emerald-200 dark:border-emerald-800">
              ...
              <span>💬 {$t.contact.chatAttached ?? 'Chat conversation attached'}</span>
