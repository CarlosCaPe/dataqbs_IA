...
def clone_or_update(repo: str, dest: Path, force: bool = False) -> None:
    ...
    if dest.exists() and any(dest.iterdir()):
        if force:
            ...
            print(f"[skip] {dest} exists and is not empty (not a git repo). Use --force to replace.")
            return
...
def main() -> int:
    ap = argparse.ArgumentParser(description='Bootstrap official SDKs into ./sdk/* from arbitraje.yaml')
    ap.add_argument('--force', action='store_true', help='Delete existing non-git directories before cloning')
    ap.add_argument('--only', type=str, default='', help='Comma-separated list of SDK names to process (e.g., binance,mexc,bitget)')
    args = ap.parse_args()
    if not CFG.exists():
    ...
    sdks = (cfg.get('sdk') or {})
    only_set = {x.strip().lower() for x in args.only.split(',') if x.strip()} if args.only else None
