...
def test_get_rate_and_qvol_direct_and_inverse():
    ...
    r, qv = mod.get_rate_and_qvol("USDT", "MINA", tickers, fee_pct=0.1, require_topofbook=False)
    assert r is not None and qv is not None
    # inverse path: MINA -> USDT should produce rate via direct MINA/USDT or inverse
    r2, qv2 = mod.get_rate_and_qvol("MINA", "USDT", tickers, fee_pct=0.1, require_topofbook=False)
...
def test_build_rates_with_min_quote_vol_and_fee():
    ...
    edges0, rmap0 = mod.build_rates_for_exchange(currencies, tickers, fee_pct=0.0, require_topofbook=False, min_quote_vol=0.0)
    edges1, rmap1 = mod.build_rates_for_exchange(currencies, tickers, fee_pct=1.0, require_topofbook=False, min_quote_vol=0.0)
...
def test_tri_worker_detects_opportunity(monkeypatch):
    ...
    class FakeEx:
        ...
        def fetch_tickers(self):
            return {
                "USDT/MINA": {"bid": 0.5, "ask": 0.51, "last": 0.505, "quoteVolume": 1000},
                "MINA/EUR": {"bid": 1.5, "ask": 1.52, "last": 1.51, "quoteVolume": 1000},
                "EUR/USDT": {"bid": 1.4, "ask": 1.42, "last": 1.41, "quoteVolume": 1000},
            }
    ...
    old_argv = sys.argv
    try:
        sys.argv = ["arbitraje.arbitrage_report_ccxt", "--mode", "tri", "--repeat", "1", "--quote", "USDT", "--tri_min_net", "0.1"]
